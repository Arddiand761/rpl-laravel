        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    @if ($errors->any())
                        <div class="relative px-4 py-3 mb-4 text-red-700 bg-red-100 border border-red-400 rounded-md"
                            role="alert">
                    ...
                    @if (session('success'))
                        <div class="relative px-4 py-3 mb-4 text-green-700 bg-green-100 border border-green-400 rounded-md shadow"
                            role="alert">
                            {{ session('success') }}
                        </div>
                    @endif
                    ...
                    <form method="POST" action="{{ route('comics.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            <label for="title"
                                class="block mb-1 text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('Judul Komik') }}</label>
                        ...
                        <div class="mb-4">
                            <label for="author"
                                class="block mb-1 text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('Penulis') }}</label>
                        ...
                        <div class="mb-4">
                            <label for="publisher"
                                class="block mb-1 text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('Penerbit') }}</label>
                        ...
                        <div class="mb-4">
                            <label for="description"
                                class="block mb-1 text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('Deskripsi') }}</label>
                           ...
                           <div class="mb-4">
                            <label for="chapter_number" class="block mb-1 text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('Nomor Chapter') }}</label>
                            <input id="chapter_number" type="text" name="chapter_number" value="{{ old('chapter_number') }}" required autofocus
                                class="w-full px-3 py-2 text-gray-900 border border-gray-300 rounded-md dark:border-gray-600 bg-gray-50 dark:bg-gray-700 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400"
                                placeholder="Contoh: 1, 2.5, Extra Chapter">
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('Bisa berupa angka (1, 2, 3) atau teks (Prolog, Epilog, Extra).') }}</p>
                        </div>

                        <div class="mb-4">
                            <label for="status"
                                class="block mb-1 text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('Status') }}</label>
                        ...
                        <div class="mb-4">
                            <label for="cover_image"
                                class="block mb-1 text-sm font-semibold text-gray-700 dark:text-gray-300">{{ __('Gambar Sampul') }}</label>
                            ...
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                                {{ __('Kosongkan jika tidak ingin mengunggah atau mengubah gambar.') }}</p>
                        </div>
